...
//  ClassicPhotos
//
//  Created by Adhyam Nagarajan, Padmaja on 7/28/17.
...
class ImageDownloader: Operation {
  ...
  override func main() {
    ...
    //4 Check for cancellation before starting. Operations should regularly check if they 
    //  have been cancelled before attempting long or intensive work.
    if self.isCancelled {
      return
    }
    //5 Download the image data
    guard let imageData = NSData(contentsOf:self.photoRecord.url as URL) else {
      return
    }
    ...
    //7 If there is data, create an image object and add it to the record, and move the 
    //  state along. If there is no data, mark the record as failed and set the appropriate 
    //  image.
